...
import { runTestDrive } from "./test-drive.js";

// Parse CLI args
const args = process.argv.slice(2);
const configPathIndex = args.indexOf("--config");
const testDriveIndex = args.indexOf("--test-drive");

// Check for test-drive mode
if (testDriveIndex !== -1) {
  await runTestDrive(args, configPathIndex, testDriveIndex);
} else {
  // Normal MCP server mode
  // Load config (optional) for normal MCP server mode
